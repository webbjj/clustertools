...
import os
import limepy
from limepy import limepy,spes
...
def get_spes(**kwargs):
    ...
    if 'M' in kwargs:
        ...
        if 'rt' in kwargs:
            ...
            smodel=spes(phi0,B=B,eta=eta,fpe=fpe,M=M,r0=1.,project=project)
    else:
        units='nbody'
    ...
    cluster=StarCluster(N,units=units,origin='cluster')
    cluster.ctype='spes'
    cluster.add_stars(np.linspace(1,N,N,dtype=int),sdata.m,sdata.x,sdata.y,sdata.z,sdata.vx,sdata.vy,sdata.vz)
    cluster.find_centre()
    cluster.key_params()
...
def get_cluster(gcname='list',source='default',mbar=0.4,names=False,params=False):
    ...
    if len(name_list) > 0 and 'list' not in gcname:
        ...
        for i in range(0,len(name_list)):
            ...
            if (source=='default' or 'deboer' in source or 'deBoer' in source) and name_list[i] in dname:
            ...
            elif (source=='default' or 'harris' in source or 'Harris' in source) and (name_list[i] in hname or name_list[i] in hname2):
...
def get_deBoer_cluster(data,gcname,mbar=0.4,names=False):
    ...
    if spes:
        ...
        N=M_pe/mbar
        cluster=get_spes(phi0=W_pe,B=B_pe,eta=eta_pe,fpe=fpe,M=M_pe,rt=rt_pe,N=N)
