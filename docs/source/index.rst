...
-- For consistancy purposes, all profile calls now return the true values of the radial bins and the calculated value. Pre-Version 1.0, some profile calls returned the natual logarithm of one (or both) values, and sometimes normalized by the cluster's effective radius. Any other returned values are returned as per their definition. For example, :math: \delta_{\alpha} is still calculated as :math: \frac{d \alpha}{d \ln(r/r_m)} even though calling ``alpha_prof`` returns ``r`` and ``alpha``.
