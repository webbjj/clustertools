...
* When ``ctype='gyrfalcon'``, the key word argument ``filename`` must also be given so ``clustertools`` knows what file to open and begin reading. The first snapshot will read in and WDunits will be convereted to kpckms. The coordinate system will be set to ``'galaxy'``.

* When ``ctype='amuse'``, the key word argument ''particles'' must be given which contains an AMUSE particle set of cluster stars. The default units are ``pckms`` but ``kpckms`` can also be selected. The default coordinate system is ``'galaxy'``, but again this can be set.

A close look at the source files will reveal a few other possible choices for ``ctype``, namely ``nbody6_webb``, ``nbody6se_webb``, ``snapauto`` and ``snaptrim``. ``nbody6_webb`` and ``nbody6se_webb`` are meant for use with my slightly customized versions of NBODY6 that output data in a specific format and are probably of little use to most users. I am working to allow users to define their own custom versions of ``load_cluster(ctype="nbdoy6")`` and ``load_cluster(ctype="nbdoy6se")`` as I am sure this is a common issue amongst NBODY6 users. ``snapauto`` and ``snaptrim`` are for internal use only, as they are not well tested and were quickly implemented to deal with the output from a custom FORTRAN code used to extra snapshots from NBODY6 and NBODY6++ binary files. I am working to implement the capability of extracting from NBODY6 and NBODY6++ binary files directly without using a FORTRAN intermediate step.

.. automodapi:: clustertools.main.load
