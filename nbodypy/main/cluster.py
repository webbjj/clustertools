...
class StarCluster(object):
    ...
    def find_centre(
        ...
        if density:
            xc, yc, zc, vxc, vyc, vzc = self.find_centre_of_density(
                xstart=xstart,
                ystart=ystart,
                zstart=zstart,
                vxstart=vxstart,
    ...
    def to_radec(self, do_key_params=False, r0=8.0, v0=220.0):
        ...
        try:
            ...
            self.r = np.sqrt(
                (self.x * np.cos(np.radians(self.xgc))) ** 2.0
                + self.y ** 2.0
                + self.z ** 2.0
            )
            self.rpro = np.sqrt(
                (self.x * np.cos(np.radians(self.xgc))) ** 2.0 + self.y ** 2.0
            )
            self.v = np.sqrt(self.vx ** 2.0 + self.vy ** 2.0 + self.vz ** 2.0)
            self.vpro = np.sqrt(self.vx ** 2.0 + self.vy ** 2.0)
            ...
            vr0, pmll0, pmbb0 = bovy_coords.vxvyvz_to_vrpmllpmbb(
                vx0, vy0, vz0, l0, b0, self.distgc, degree=True
            )
    ...
    def to_centre(
        self, do_order=False, do_key_params=False, orthographic=False, VandeVen=False
    ):
        """
        NAME:
        ...
        if self.origin != "centre":
            ...
            if self.origin != "cluster":
                self.to_cluster(
                    do_key_params=False, orthographic=orthographic, VandeVen=VandeVen
                )
            ...
            if self.units == "radec" and self.origin == "sky":
                if orthographic:
                    ...
                    if VandeVen:
                        self.x = (
                            (10800.0 / np.pi)
                            * np.cos(np.radians(self.y))
                            * np.sin(np.radians(self.x) - np.radians(self.xgc))
                            / 60.0
                        )
                        self.y = (
                            (10800.0 / np.pi)
                            * (
                                np.sin(np.radians(self.y))
                                * np.cos(np.radians(self.ygc))
                                - np.cos(np.radians(self.y))
                                * np.sin(np.radians(self.ygc))
                            )
                            * np.cos(np.radians(self.x) - np.radians(self.xgc))
                            / 60.0
                        )
                    else:
