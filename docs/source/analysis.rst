...
Finally, cluster properties that depend on the external tidal field, like its tidal radius (also referred to as the Jacobi radius) or limiting radius (also referred to as the King tidal radius), can be calculated when the orbit and external tidal field are given. The default external ``galpy`` potential is always ``None``. A simple Galactic model to use would be the ``MWPotential2014`` model, which is an observationally motivated representation of the Milky Way that can be imported via:
...
Since ``plot=True``, a figure showing the cluster's density profile and the background density in MWPotential at the cluster's position is returned in order to view exactly where the limiting radius is. cluster.rl is now set equal to the measured limiting radius. See the full list of functions and their input parameters to see what other functions have the ``plot`` option as well.
...
It is, however, worth noting that ``rtidal`` has two additional arguments that can improve your calculation of the tidal radius. It may be the case that not all of your particles are technically part of the star cluster. In this case, you could consider setting ``rtiterate`` and ``rtconverge``. When set, ``clustertools`` will first calculate ``rt`` using all the particles in the ``StarCluster``. It will then recalculate ``rt`` using only the particles within the initial estimate of the tidal radius. This process will occur ``rtiterate`` times or until the ratio of the new tidal radius to the previous tidal radius is greater than ``rtconverge``. For example, ``rtconverge=0.9`` means the tidal radius has changed by less than 10 percent between iterations. Alternatively, if one knew which stars were bound to the cluster you could define a ``sub_cluster`` of bound particles and calculate its tidal radius or pass a boolean array to ``rtidal`` to inform it which stars to use in the calculation of ``rt``.


All functions, with the exception of calculating the cluster's tidal radius, can be called using projected values with ``projected=True``. When called internally, a function call will default to whatever ``StarCluster.projected`` is set to if ``projected`` is not given.
...
Note, when measuring the radial variation in the mass function or the degree of energy equipartition, ``delta_alpha`` and ``delta_eta`` are calculated with clustercentric radii normalized by the effective radius. Whether or not ``normalization`` is True or False only affects whether the returned radial bins are normalized or not.
...
In cases where the ``StarCluster`` does not evolve in isolation, it is possible to specify both the cluster's orbit and the external tidal field. When orbital and tidal field information are provided, ``clustertools`` makes use of ``galpy`` to help visualize and analyse the cluster's orbital properties. Hence Bovy (2015) must be cited anytime a ``clustertools`` function makes use of the external tidal field and/or the cluster's orbit.
...
Similar to functions, orbital analysis can be done using internal calls (which set variables inside the StarCluster class) or externally (which returns information). For example, one can easily initialize a cluster's ``galpy`` orbit, given its galactocentric coordinates are known, using:
...
``galpy`` orbits can be initialized for individual stars as well via ``cluster.initialize_orbits()``. If ``cluster.origin`` is ``centre`` or ``cluster``, the orbits are initialized with clustercentric coordinates. If ``cluster.origin='galaxy'`` then each star's position in the galaxy is used to initialize the orbit.
...
Stars can also be matched to a point along the orbital path, where each star's distance from the path and distance along the path to the progenitor is calculated. This function effectively creates a convenient frame of reference for studying escaped stars.
...
By default, all stars are simply shifted so the cluster is centred on its new galactocentric position. If one wishes to identify tail stars that have their own orbits interpolated, set ``do_tails=True`` as above and provide a criterion for what defines a tail star. In the above example, any star beyond 0.1 ``cluster.units`` is a tail star. Other criteria include velocity, energy, kwtype, or a custom boolean array (similar to defining subsets for profiles as discussed above).

If one wishes to interpolate the orbits of stars within the cluster, then use ``orbits_interpolate`` with ``cluster.origin`` equal to centre or cluster. In this case the ``galpy`` potential of the cluster should be provided. A recommended potential would be a Plummer sphere
...
Stars which escape a cluster lead to the formation of tidal tails, which survive as stellar streams long after the cluster has dissolved. For star cluster simulations that keep track of a cluster's tidal tails, a few methods are included in ``clustertools`` to help in their analysis. Since these methods require knowledge of the cluster's orbit, they accept ``ro``, ``vo``, ``zo``, and ``solarmotion`` as inputs if the values set for the ``StarCluster`` are not relevant. Bovy (2015) should also be cited when using tail functions as well.

The first is the ``to_tail`` operation, which rotates the system such that the cluster's velocity is pointing along the positive x-axis. Calling the operation externally returns each star's coordinates in the rotated system:

>>> x,y,z,vx,vy,vz=to_tail(cluster)
