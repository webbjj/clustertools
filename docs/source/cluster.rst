...
``StarCluster`` accepts additional optional arguments, each of which have defaults. They are the current time (``tphys=0``), the units (``units=None``) and origin (``origin=None``) of the coordinate system and the name of the code used to generate the dataset ``ctype``. The units and origin variables only need to be specified if unit or coordinate transformations are going to be done (see :ref:`Units <cluster_units>` and :ref:`Coordinate Systems <cluster_coordinate_systems>` for more information). The code type ``ctype`` defaults to ``'snapshot'``, but can alternatively be set to ``'astropy_table'``, ``'amuse'``, ``'galpy'``, ``'gyrfalcon'``, ``'limepy'``, ``'nbody6'``, ``'nbody6pp'``. ``ctype`` informs the ``StarCluster`` of the input files format. See :ref:`Loading and Advancing <load_and_advance>` for more information on how ``ctype`` is used. Other keywords accepted when initializing a ``StarCluster`` can be found in the complete documentation (see StarCluster).
...
``add_binary_stars`` has the same features as ``add_stars`` in that masses (``mb1``, ``mb2``), ids (``id1``, ``id2``), initial masses (``m01``, ``m02``), and population numbers (``npop1``, ``npop2``) can be provided.
...
For those not familiar with ``NBODY6``, please consult the documentation for ``add_nbody6``, ``add_sse``, and ``add_bse`` for the definition of each variable. It is important to note that each of the above variables are initialized upon the initialization of ``StarCluster``, hence they can be set manually as well if you are using a code other than ``NBODY6`` and would like to define some of these parameters. Note, no units or origin are associated with any of the values provided via ``add_nbody6``, ``add_sse``, and ``add_bse`` such that they are not adjusted when unit and coordinate transformations are performed. 
...
>>> cluster.subset(rmin=0.,rmax=1.,mmin=0.1,mmax=1.0,vmin=-5.,vmax=5.,emin=-100,emax=100,kwmin=0,kwmax=1.,projected=False)
...
Instead of using ``subset``, it's also possible to extract a subset of stars from a ``StarCluster`` to form a new ``StarCluster`` using the ``sub_cluster`` function. For example, to extract only stars within the cluster's half-mass radius one can call:
...
Finally, it is also possible to determine where two different ``StarCluster``s overlap. This function is useful when trying to compare two different systems. The comparison can be done via:
...
In order to convert between certain units, it is necessary to know the distance to the Galactic Centre (``ro``), the rotation velocity at ``ro`` (``vo``), the Sun's height above the Galactic disk (``zo``), and the Sun's motion with respect to the local standard of rest (``solarmotion``). When a ``StarCluster`` is initialized, the default value for ``ro`` is 8.275 kpc (Gravity Collaboration, Abuter, R., Amorim, A., et al. 2020 ,A&A, 647, A59), the [U,V,W] motion of the Sun is set to [-11.1,12.24,7.25] (Schönrich, R., Binney, J., Dehnen, W., 2010, MNRAS, 403, 1829) and the velocity of the local standard of rest is 239.23 km/s. The choice of ``vo`` is such that ``vo`` + V is consistent with current estimates of the proper motion of Sagitarius A* (Reid, M.J. & Brunthaler, A., ApJ, 892, 1). The height of the Sun above the disk is 0.0208 kpc (Bennett, M. & Bovy, J. 2019, MNRAS, 483, 1417). However each of these values can be changed when initializing a ``StarCluster``.
