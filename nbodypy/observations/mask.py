...
class ObservationalMask(object):
    ...
    def apply_mask(self):
        ...
        if self.name == "m15" or self.name == "M15":
            ...
            q1indx = (
                ((self.cluster.x / self.cluster.rm) >= q1lims[0])
                * ((self.cluster.x / self.cluster.rm) <= q1lims[1])
                * ((self.cluster.y / self.cluster.rm) >= q1lims[2])
                * ((self.cluster.y / self.cluster.rm) <= q1lims[3])
            )
            q2indx = (
                ((self.cluster.x / self.cluster.rm) >= q2lims[0])
                * ((self.cluster.x / self.cluster.rm) <= q2lims[1])
                * ((self.cluster.y / self.cluster.rm) >= q2lims[2])
                * ((self.cluster.y / self.cluster.rm) <= q2lims[3])
            )
            q3indx = (
                ((self.cluster.x / self.cluster.rm) >= q3lims[0])
                * ((self.cluster.x / self.cluster.rm) <= q3lims[1])
                * ((self.cluster.y / self.cluster.rm) >= q3lims[2])
                * ((self.cluster.y / self.cluster.rm) <= q3lims[3])
            )
            q4indx = (
                ((self.cluster.x / self.cluster.rm) >= q4lims[0])
                * ((self.cluster.x / self.cluster.rm) <= q4lims[1])
                * ((self.cluster.y / self.cluster.rm) >= q4lims[2])
                * ((self.cluster.y / self.cluster.rm) <= q4lims[3])
            )

            xhull = (
                np.array(
                    [
                        -0.025200275014222413,
                        -0.023248840101820767,
                        -0.022252171603265446,
                        -0.020876374968530564,
                        -0.019768232948539806,
                        -0.019516293671548123,
                        -0.018310827507562306,
                        -0.016141939740057017,
                        -0.01507099008868237,
                        0.020622932660628257,
                        0.030769270910864063,
                        0.03402070578819945,
                        0.034327150432339686,
                        0.03464686440992152,
                        0.034355000816019575,
                        0.03374778542629317,
                        0.02442140775679065,
                        0.024403514928413285,
                        0.023904902487531996,
                        0.022511976486394615,
                        0.020470072739238877,
                        0.0188020846312626,
                        0.017870995541351138,
                        0.017591948439801587,
                        0.008300117344556341,
                        0.007085393874204251,
                        -0.009972763393508404,
                        -0.011261330698878028,
                        -0.02214748809652206,
                        -0.026122585612488373,
                        -0.03346958022086817,
                        -0.03399466578178241,
                        -0.03309639437024884,
                        -0.03100587078182435,
                        -0.02904970980645645,
                        -0.028262363464591378,
                        -0.027903603829392717,
                        -0.025745234255737832,
                    ]
                )
                / self.rm
            )
            yhull = (
                np.array(
                    [
                        ...
                        -0.03479537796031218,
                        -0.03535176259000022,
                        -0.035427181195303775,
                        -0.03539306698617067,
                        -0.03488420765700969,
                        -0.034618570591543,
                        -0.025546899011511297,
                        -0.022961518217483424,
                        -0.021911221962715968,
                        -0.021612782576963474,
                        -0.02111804111942903,
                        -0.01924967910897742,
                        -0.01598325703837688,
                        0.03095362077113272,
                        ...
                        0.03242387358258812,
                        0.03366825171526268,
                        0.034802686363492465,
                        0.03536156299431959,
                        0.03552819870817594,
                        0.035491980093389976,
                        0.03296722781833852,
                        0.03263449270445504,
                        0.027920886042690843,
                        0.02755543757181496,
                        0.02444082224849501,
                        0.023292683397820806,
                        0.021116007388034132,
                        0.02039697408638987,
                        0.013997660928067528,
                        0.0012260088299883426,
                        -0.010216512222578881,
                        -0.014745097038631718,
                        -0.016765934883835022,
                        -0.02861945184298367,
                    ]
                )
                / self.rm
            ...
            hindx = inhull(
                xhull,
                yhull,
                self.cluster.x / self.cluster.rm,
                self.cluster.y / self.cluster.rm,
            )

            self.oindx = np.logical_or(
                np.logical_or(
                    np.logical_or(q1indx, q2indx), np.logical_or(q3indx, q4indx)
                ),
                hindx,
            )
    ...
    def load_rbins(self):
        ...
        if self.name == "m15" or self.name == "M15":
            self.r_lower = (
                np.array(
                    [25.0, 40.0, 55.0, 70.0, 85.0, 250.0, 350.0, 450.0, 550.0, 650.0]
                )
                / 3600.0
            )
            self.r_upper = (
                np.array(
                    [40.0, 55.0, 70.0, 85.0, 100.0, 350.0, 450.0, 550.0, 650.0, 750.0]
                )
                / 3600.0
            )
        elif self.name == "m30" or self.name == "M30":
            self.r_lower = (
                np.array([10.0, 20.0, 40.0, 200.0, 250.0, 350.0, 650.0]) / 3600.0
            )
            self.r_upper = (
                np.array([20.0, 40.0, 100.0, 250.0, 350.0, 650.0, 1000.0]) / 3600.0
            )

        self.r_mean = (self.r_lower + self.r_upper) / 2.0
    ...
    def load_mbins(self):
        ...
        self.m_mean = (self.m_lower + self.m_upper) / 2.0
