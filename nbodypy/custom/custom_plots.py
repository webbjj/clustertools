...
from ..main.profiles import m_prof
...
def aplot(
    data, nsnap=0, tsnap=None, prefix="", nmass=10, nrad=20, filename=None, **kwargs
):
    ...
    if rfrac:
        lrprof = np.linspace(0.0, 1.0, len(aprof))
        xlabel = r"$r_n$"
    else:
        xlabel = r"ln(r/$r_m$)"
    ...
    out = nlplot(
        lrprof, aprof, xlabel=xlabel, ylabel=r"$\alpha$", filename=filename, **kwargs
    )
