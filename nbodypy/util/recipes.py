...
def area_enclosed(x,y,thresh=None,nrand=1000,method='sphere',full=False,plot=False):
    """
    NAME:
    ...
    if full:
        ...
        dr=np.sqrt((xrfull-xfull)**2.+(yrfull-yfull)**2.)
        dr=dr.reshape(len(x),nrand)
        drmin=np.amin(dr,0)
    else:
        for i in range(0,nrand):
            dr=np.sqrt((xrand[i]-x)**2.+(yrand[i]-y)**2.)
