...
at a later point in time if the cluster was populated manually. For computational efficiency, ``sortstars`` can be set to ``False`` when sorting is not required.
